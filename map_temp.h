...
#define DATA_MAX_T 21 // 最大ステージ数

class map_temp : public Scene
{

private:
	int MapDeta[DATA_MAX_T]; // マップ内部データ
	...
	int icon_loc[DATA_MAX_T][2];         // アイコン座標
	int icon_vec;                      // アイコン移動量
	int total_vec;                     // 総合移動量

	// テンプレート調整用変数
	int icon_loc_change[DATA_MAX_T][2];  // 変更後デフォルトアイコン座標
	int change_icon;                   // 移動させるアイコン
	int change_icon_tab;               // 移動させるアイコン(十の位)

	// 次に行けるステージ(最後は1番最初の選択ステージ)
	const int next_stage[DATA_MAX_T][3]{
		{3,-1},{4,5,-1},{5,6,-1},{7,-1},
